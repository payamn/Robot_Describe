...
class DataSet:
    def __init__(self, directions, data_generation, is_data_generation, batch_size = 1, bag_number = 0, use_direction_file=True, read_lang_disk=True):
        ...
        if is_data_generation:
            ...
            script.rospy.loginfo("number of bag read: " + self._bag_name + str(self._bag_num + 1))
    ...
    def read_bag(self):
        ...
        for file in files:
            ...
            try:
                ...
                script.rospy.logerr("bag %s problem skipping:", file)
                continue
            ...
            for topic, msg, t in bag.read_messages(topics=['language', '/robot_0/base_scan_1', '/robot_0/speed']):
                if topic == 'language':
                    ...
                    if len(data.laser[0])==0:
                        script.rospy.logerr("file without laser data:%s skipping", file)
                        is_fail = True
                        break
                    counter += 1
            ...
            if self._bag_num%100==0:
                script.rospy.loginfo("read %d bags", self._bag_num)
            self._bag_num += 1
            bag.close()
        ...
        script.rospy.logwarn("number of bag read: " + str(self._bag_num) + "laser: " + str(counter))
    # while True:
    #         try:
    #
    #
    ...
    def new_sequence(self):
        self.lock.acquire()
        script.rospy.logwarn("saving " + str(self._bag_name) + str(self._bag_num))
        self._bag.close()
        self._bag_num += 1
        self._bag = rosbag.Bag(self._bag_name + str(self._bag_num), 'w')
        self.lock.release()
